                    ...
                    <ol class="breadcrumb m-0 p-0 d-flex align-items-center">
                        <template v-for="(item, index) in breadcrumbs" :key="index">
                            <li class="breadcrumb-item d-flex align-items-center">
                                <Link v-if="item.label === 'icon-home' && item.url" :href="item.url" class="text-decoration-none">
                                ...
                                <Link v-else-if="item.url" :href="item.url" class="text-decoration-none" :class="{ current: item.current }">{{ item.label }}</Link>
                                <span v-else :class="{ current: item.current }">{{ item.label }}</span>

                                <svg v-if="index < breadcrumbs.length - 1" width="12" height="20" viewBox="0 0 12 20" class="mx-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 14 14" focusable="false" aria-hidden="true" class="a8x1wuy a8x1wux _1fragem32 _1fragemq1 _1fragemly _1fragemlo _1fragemp6"><path stroke-linecap="round" stroke-linejoin="round" d="m4.75 2 4.646 4.646a.5.5 0 0 1 0 .708L4.75 12"></path></svg>
                                </svg>
                            </li>
                        </template>
                    </ol>
